...
ccCols <- c("Name","Avg SAT","SATVR75","SATMT75","SATWR75", "Admission Rate","Cost Annual","Family Income","InStateTuition", "OutStateTuition", "Latitude", "Longitude")
cc1 <- ccA[ccCols]

```
After eliminating non-accredited schools and schools which are vocational, offer only associate's degrees, and otherwise did not fit in the range of 4 year bachelor degree issuing schools, I was left with 1,165 observations.
...
### Select a subset of the available columns:
#ccColsC <- c("Name","Avg SAT","Tot3SAT75","Cost Annual","Family Income","InStateTuition", "OutStateTuition", "Latitude", "Longitude","SATMT75","SATVR75","SATWR75")
#cc1c <- ccA[ccColsC]
...
hist(cc2$`Cost Annual`,
     main = "Average Annual Cost for All Colleges (Tuition + Housing + Food)",
     xlab = "Annual Cost")
```
...
##### The summary below shows that the highest score for each of the 3 sections: reading, math and writing, is 800.  Highest total score for all 3 is 2400.  The highest total score for reading and math without writing is 1600.
...
### Group 1:  Top 90th percentile of the top SAT scores combined: 2160, Orderd by SAT Scores
...
ccTopTenth <- subset(ccTopTenth, ccTopTenth$`SAT Avg` >= 2160)
...
kable(ccTopTenthD, "html") %>%
  ...
  add_header_above(c(" " = 1, "Group 1 Colleges Ordered by SAT Score" = 4)) %>% 
...
kable(ccTopTenthE, "html") %>%
  kable_styling("striped", "responsive", full_width = F, font_size = 14) %>%
   column_spec(2, width = "30em")  %>% 
  add_header_above(c(" " = 1, "Group 1 Colleges Ordered by Average Annual Cost" = 4)) %>% 
 scroll_box(width = "100%", height = "400px")
```
```{r}
...
kable(ccTopTenthF, "html") %>%
  kable_styling("striped", "responsive", full_width = F, font_size = 14) %>%
   column_spec(2, width = "30em")  %>% 
  add_header_above(c(" " = 1, "Group 1 Colleges Ordered by Average Family Income" = 4)) %>% 
 scroll_box(width = "100%", height = "400px")
```



#### Location of Group 1 Colleges
#####Click on the map pin to see the school name.  
...
content <- c(ccTopTenth$Name,"<br>",
          "SAT: ",ccTopTenth$`SAT Avg`,"<br>",
          "Cost: ",ccTopTenth$`Cost Annual`,"<br>",
          "Family Income: ",ccTopTenth$`Family Income`)

leaflet(data = ccTopTenth) %>%
addTiles() %>% 
addProviderTiles(providers$OpenStreetMap) %>% 
addPopups(content) %>%
addMarkers(  lng = ccTopTenth$Longitude, lat = ccTopTenth$Latitude) 
...
```
#### Summary of the Top 10th percentile colleges. 
...
##### CONCLUSION:  
It's expensive to go to most top tenth percentile schools. Most cost around $60,000-70,000 per year but there are several options that offer lower annual cost especially if you qualify for In-State tuition.
...
```
##### CONCLUSION:  
In the 75-90th percentile, there is a wide range of costs and seemingly no correlation between annual cost and SAT scores.  
...
##### CONCLUSION:  
In State tuition at Public Universities offers the lowest annual cost. Students who rank in the 75-90th percentile can get a better cost value by going to a public university in their state if that is an option.  
